...
Jeu::Jeu() : pop{Population()}, grille{Grille()} {
  int alea = 0;
...
Population Jeu::getPop() const { return pop; }

Grille Jeu::getGrille() const { return grille; }
...
TEST_CASE("VoisinsVides + VoisinsAnimal") {
  ...
  CHECK_THROWS_AS(game.voisinsAnimal(Espece::Vide, Coord({1, 1})),
                  invalid_argument);
...
bool Jeu::deplaceAnimal(Animal a)
{
        ...
        else if(a.getEspece() == Espece::Renard)
        {
                ...
                {
                        int indice = evLapin.getCase(rand() %
evLapin.cardinal()); Coord cApres = Coord{indice};
        ...
        {
                throw invalid_argument("On ne peut pas deplacer un Animal
Vide");
        }
...
void Jeu::affiche() const {
  ...
  cout << afficheNombreAnimal();
}

string Jeu::afficheNombreAnimal() const {
  ostringstream out;
  ...
  for (int i = 0; i < e.cardinal(); i++) {
    if (pop.get(grille.getCase(Coord{e.getCase(i)})).getEspece() ==
        Espece::Lapin) {
      nbLapin++;
    } else if (pop.get(grille.getCase(Coord{e.getCase(i)})).getEspece() ==
               Espece::Renard) {
      nbRenard++;
    }
  }
  ...
  for (int i = 0; i < TAILLEGRILLE; i++) {
    for (int j = 0; j < TAILLEGRILLE; j++) {
      if (grille.getCase(Coord{i, j}) != -1) {
        ...
        if (not(cPop == cGrille)) {
          throw runtime_error(
              "L'animal n'est pas stocke a la place ou il doit l'etre");
        }
      }
    }
  }
